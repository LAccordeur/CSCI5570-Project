...
import com.rogerguo.spatial.geomesa.common.CommandLineDataStore;
...
public abstract class GeoMesaClient{
    ...
    public NearestNeighbors runKNN(String featureName, int k, double guessedDistance, double maxLimitDistance, Query query, SimpleFeature searchPoint) {
        DataStore datastore = null;
        try {
            datastore = createDataStore(params); //创建一个HBaseDataStore实例
            ...
            NearestNeighbors result = KNNQuery.runNewKNNQuery(source, query, k, guessedDistance, maxLimitDistance, searchPoint);
            long stop = System.currentTimeMillis();
            System.out.println("KNN takes " + (stop - start) + " ms");
            System.out.println("Done");
            return result;
    ...
    public void runRangeQuery(Query query) {
        DataStore datastore = null;
        int n = 0;
    ...
    public void ensureSchema(DataStore datastore, CommonData data) throws IOException {
        ...
        if (sft == null) {
            throw new IllegalStateException("Schema '" + data.getTypeName() + "' does not exist. " +
                                            "Please execute the associated QuickStart to generate the test data.");
        }
    }
